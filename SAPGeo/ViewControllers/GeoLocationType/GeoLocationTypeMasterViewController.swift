...
class GeoLocationTypeMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    var sapGeoService: SAPGeoService<OfflineODataProvider>!
    public var loadEntitiesBlock: ((_ completionHandler: @escaping ([GeoLocationType]?, Error?) -> Void) -> Void)?
    private var entities: [GeoLocationType] = [GeoLocationType]()
    private let logger = Logger.shared(named: "GeoLocationTypeMasterViewControllerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let geolocationtype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ID", value: "\(geolocationtype.id!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.sapGeoService.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! GeoLocationTypeDetailViewController
            ...
            detailViewController.sapGeoService = self.sapGeoService
            ...
            let detailViewController = dest.viewControllers[0] as! GeoLocationTypeDetailViewController
...
extension GeoLocationTypeMasterViewController: EntitySetUpdaterDelegate {
