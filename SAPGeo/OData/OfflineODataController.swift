...
public class OfflineODataController {
    ...
    public func configureOData(sapURLSession: SAPURLSession, serviceRoot: URL, onboardingID: UUID) throws {
        ...
        self.sapGeoService = SAPGeoService(provider: offlineODataProvider)
    }
    ...
    public func openOfflineStore(synchronize: Bool, completionHandler: @escaping (Error?) -> Void) {
        if !self.isOfflineStoreOpened {
            ...
            self.sapGeoService.open { error in
                if let error = error {
    ...
    private func configureDefiningQueries(on offlineODataProvider: OfflineODataProvider) throws {
        ...
        do {
            try offlineODataProvider.add(definingQuery: OfflineODataDefiningQuery(name: SAPGeoServiceMetadata.EntitySets.geoLocation.localName, query: DataQuery().from(SAPGeoServiceMetadata.EntitySets.geoLocation).selectAll().top(20), automaticallyRetrievesStreams: false))
        } catch {
    ...
    private func downloadOfflineStore(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.sapGeoService.download { error in
            if let error = error {
    ...
    private func uploadOfflineStore(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.sapGeoService.upload { error in
            if let error = error {
